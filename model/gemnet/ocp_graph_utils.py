...
def custom_segment_coo(index, dim_size):
    # 初始化一个全是零的张量，大小为 dim_size，用来存储每个节点的邻居数量
    num_neighbors = torch.zeros(dim_size, dtype=torch.long, device=index.device)
    
    # 对每个目标节点的索引进行遍历，在对应位置增加邻居计数（默认为1）
    num_neighbors.index_add_(0, index, torch.ones_like(index, dtype=torch.long))
    
    return num_neighbors
def custom_segment_csr(src, indptr, reduce='sum'):
    """
    Custom implementation of segment_csr for sum reduction
    
    Args:
        src: Input tensor (1D)
        indptr: Compressed row pointers that define the segments
        reduce: Reduction operation (only 'sum' implemented here)
    
    Returns:
        Tensor with the reduced values for each segment
    """
    if reduce != 'sum':
        raise NotImplementedError("Only 'sum' reduction is implemented in this custom function")
    
    # Initialize output with zeros
    out_size = indptr.size(0) - 1
    out = torch.zeros(out_size, dtype=src.dtype, device=src.device)
    
    # Perform the segmented sum
    for i in range(out_size):
        start = indptr[i]
        end = indptr[i+1]
        if start != end:  # Only sum if segment is not empty
            out[i] = src[start:end].sum()
...
def get_max_neighbors_mask(
    ...
    # ones = torch.ones(index.shape, dtype=index.dtype, device=device)
    num_neighbors = custom_segment_coo(index, dim_size=num_atoms)
    ...
    num_neighbors_image = custom_segment_csr(num_neighbors_thresholded, image_indptr)
